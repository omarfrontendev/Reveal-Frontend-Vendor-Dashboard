...
export const useUsersTableLogic = () => {
    ...
    const [tableOptions, setTableOptions] = useState<UsersTableOptions>({
        ...
        roles: mobileUserRoles.map(role => role.value),
    });
    ...
    const statusFilter = useMemo(
        () => userStatus.find((option) => option.value === tableOptions.isActive)?.value,
        [tableOptions.isActive]
    );

    // Memoized current selected status value for UI display
    const roleFilter = useMemo(
        () => tableOptions.roles?.length === 1 && mobileUserRoles.find((option) => option.value === tableOptions.roles[0])?.value,
        [tableOptions.roles]
    );
