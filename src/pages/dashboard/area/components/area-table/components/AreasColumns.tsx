...
import { AreaActionsCell } from "./AreaActionCell";

export const useAreasColumns = (setRefreshData: any) => {
  ...
  return [
    ...
    },
    {
      ...
      cell: ({ row }: any) => <div>{row.original.areaCode}</div>,
      enableSorting: false,
      size: 200,
      meta: {
        headerClassName: '',
    ...
    },
    {
      id: 'actions',
      cell: ({ row }: any) => (
        <AreaActionsCell row={row} setRefreshData={setRefreshData} />
      ),
      enableSorting: false,
      size: 50,
      meta: {
