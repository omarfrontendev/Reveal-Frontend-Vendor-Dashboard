...
import { checkEmailState, loginUser, requestOTP, verfiyOTP } from '@/app/store/features/auth/authThunk';
...
export function LoginPage() {
  ...
  const handleCheckEmailState = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);

    try {
      const res = await dispatch(checkEmailState({ email })).unwrap();
      ...
      if (!isActive) {
        ...
        return;
      } else if (isFirstActivationPending) {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t("login.msgSuccess"));
      navigate('/');
    } catch (error: any) {
            ...
            <div className="relative">
              ...
              <div className="mb-8">
                ...
                <h3 className="text-2xl text-white mb-2">
                  {show2FA ? t("login.twoFactorTitle") : t("login.signIn")}
                </h3>
                <p className="text-sm text-slate-400">
              ...
              <form onSubmit={showPassField ? handleLogin : show2FA ? verifyOTP : handleCheckEmailState} className="space-y-6">
                {showPassField ? (
                  <>
                    ...
                    <Button
                      ...
                      onClick={() => setShowPassField(false)}
                    >
                ...
                <Button
                  type="submit"
                  disabled={isLoading}
                  className="mb-0 w-full h-12 bg-gradient-to-r from-purple-600 to-pink-600 text-white hover:shadow-lg hover:shadow-purple-500/50 transition-all disabled:opacity-70 disabled:cursor-not-allowed"
                >
                  {isLoading
                    ? t("login.processing")
                    : show2FA || showPassField
                      ? t("login.signIn")
                      : t("login.next")}
                </Button>

                <Link to="/auth/forget-password" className='text-white mt-2 mb-4 block text-end text-sm'>{t("login.forgetPassword")}</Link>
